...
bool twi_master_read(uint8_t *data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    NRF_TWI1->TASKS_STARTRX = 1;
		...
		{
			repeted_start_flg = true;
			data_length=0;
		}		
		
		else
		{
			//repeted_start_flg=false;
			while(true)
			{
		...
		//  NRF_PPI->CHENCLR = PPI_CHENCLR_CH0_Msk;
			sd_ppi_channel_enable_clr(PPI_CHEN_CH3_Msk);
			return true;
		}
}
...
bool twi_master_init(void)
{
    ...
    */
    ...
    err_code = sd_ppi_channel_enable_clr(PPI_CHEN_CH3_Msk);
    ASSERT(err_code == NRF_SUCCESS);
