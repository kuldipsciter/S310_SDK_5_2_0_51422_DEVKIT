...
uint16_t updated_intensity  = 0;
bool pwm_enable = false;
void set_intensity(int intensity_value)
{
	if(intensity_value ==0)    //for zero intensity, It will disable gpioet channel
	{
		pwm_enable = false;
	...
	{
		if(pwm_enable == false)    //If gpiot is disabled, Enable it. 
		{
			pwm_enable = true;
			nrf_pwm_set_enabled(true);
		}
		if(intensity_value == 100)     //for max intensity, It will set intensity to (pwm_max_value - 1)
		{
			updated_intensity = pwm_max_value - 1;
		}
		else
		{
			updated_intensity = (int)(10*intensity_value);  //for (1 - 99) intensity, It will set intensity to (intensity*10)
		}
	}
}
...
void PWM_IRQHandler(void)
{
    ...
    PWM_TIMER->CC[3] = pwm_max_value = pwm_next_max_value;   //set cc[3] value to max 100
    for(i = 0; i < pwm_num_channels; i++)
    {
		  PWM_TIMER->CC[i] = pwm_next_value[i];             //set cc[0] value depends on intensity
          if(!pwm_running[i])
           {
               nrf_gpiote_task_config(pwm_gpiote_channel[i], pwm_io_ch[i], NRF_GPIOTE_POLARITY_TOGGLE, NRF_GPIOTE_INITIAL_VALUE_HIGH);  
			   pwm_running[i] = 1;
           }
		   nrf_pwm_set_value(i, updated_intensity);			//update sample value for next inteval.
    }
    PWM_TIMER->SHORTS = TIMER_SHORTS_COMPARE3_CLEAR_Msk;
    PWM_TIMER->TASKS_START = 1;
}
